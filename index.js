...
app.get(
  ...
  checkAPI_key,
  checkAdminAuth,
  checkRole(["SUPER_ADMIN", "COMMANDER", "REGISTRAR", "VISITATION"]),
  async (req, res) => {
    const connection = await db.getConnection();
    try {
      ...
      return res
        .status(500)
        .json({ message: "เกิดข้อผิดพลาดในการดึงข้อมูลญาติ" });
    } finally {
...
app.put(
  ...
  checkAPI_key,
  checkAdminAuth,
  checkRole(["SUPER_ADMIN", "REGISTRAR"]),
  async (req, res) => {
    const connection = await db.getConnection();
    try {
      ...
      if (is_active === undefined || ![0, 1].includes(Number(is_active))) {
        return res
          .status(400)
          .json({ message: "กรุณาส่งค่า is_active เป็น 0 หรือ 1 เท่านั้น" });
      }
      ...
      await connection.execute(
        `UPDATE user SET is_active = ? WHERE userId = ?`,
        [is_active, targetUserId],
      );

      const statusMessage =
        is_active == 1 ? "เปิดใช้งานบัญชี" : "ระงับการใช้งานบัญชี";
      ...
      return res
        .status(500)
        .json({ message: "เกิดข้อผิดพลาดในการอัปเดตสถานะบัญชี" });
    } finally {
      if (connection) connection.release();
    }
  },
    ...
    try {
      ...
      if (!firstname || !lastname || !phone) {
        return res
          .status(400)
          .json({
            message: "กรุณากรอกชื่อ นามสกุล และเบอร์โทรศัพท์ให้ครบถ้วน",
          });
      }
      const [prefixRows] = await connection.execute(
        `SELECT id_prefixes, prefixes_nameTh FROM prefixes`,
      );
      const prefixMap = {};
      ...
      const [checkPhone_SQL] = await connection.execute(
        "SELECT phone FROM user WHERE phone = ? AND userId != ?;",
        [phone, targetUserId],
      );
      ...
      return res
        .status(200)
        .json({ message: "แก้ไขข้อมูลส่วนตัวญาติเรียบร้อยแล้ว" });
    } catch (error) {
      ...
      return res
        .status(500)
        .json({ message: "เกิดข้อผิดพลาดในการแก้ไขข้อมูลญาติ" });
    } finally {
      if (connection) connection.release();
    }
  },
    ...
    try {
      ...
      if (
        ...
        throw new ValidationError(
          "กรุณาอัปโหลดรูปภาพบัตรประชาชนและรูปภาพเซลฟี่",
          400,
        );
      }
      ...
      const [relationRows] = await db.execute(
        "SELECT id FROM relation_type WHERE type_name_relationship_th = ?",
        [trimmedRelation],
      );

      if (relationRows.length === 0) {
        throw new ValidationError(
          `ไม่พบประเภทความสัมพันธ์ '${trimmedRelation}' ในระบบ กรุณาตรวจสอบอีกครั้ง`,
          400,
        );
      }
      ...
      const [userRows] = await db.execute(
        "SELECT id_card, firstname, lastname FROM user WHERE userId = ?",
        [userId],
      );
      ...
      const [inmateRows] = await db.execute(
        `SELECT id FROM inmate WHERE firstname = ? AND lastname = ?`,
        [firstname, lastname],
      );
      if (inmateRows.length === 0) {
        throw new ValidationError(
          "ไม่พบข้อมูลรหัสผู้ต้องขังนี้ในระบบ กรุณาตรวจสอบชื่อนามสกุลอีกครั้ง",
          404,
        );
      }
      ...
      if (existingReq.length > 0) {
        ...
        if (rel.status === "APPROVED" || rel.status === "PENDING") {
          throw new ValidationError(
            "คุณได้ส่งคำขอไปแล้ว หรือสถานะได้รับการอนุมัติแล้ว ไม่สามารถส่งซ้ำได้",
            400,
          );
        }
      }
      ...
      const [idCardFilename, selfieFilename] = await Promise.all([
        ...
        processAndSaveImage(
          req.files["selfie_image"][0].buffer,
          "selfie_image",
        ),
      ]);
...
app.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: "เกิดข้อผิดพลาดในการดึงข้อมูลเจ้าหน้าที่" });
    } finally {
      if (connection) connection.release();
    }
  },
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!username || !password || !fullname || !role) {
        return res
          .status(400)
          .json({ message: "กรุณากรอกข้อมูลที่จำเป็นให้ครบถ้วน" });
      }
      ...
      if (!allowedRoles.includes(role)) {
        return res
          ...
          .json({
            message:
              "role ของคุณไม่ถูกต้องต้องเป็น !'REGISTRAR', 'VISITATION', 'COMMANDER', 'SUPER_ADMIN'",
          });
      }
      ...
      if (existingUser.length > 0) {
        return res
          .status(400)
          .json({ message: "Username หรือรหัสเจ้าหน้าที่นี้ มีในระบบแล้ว" });
      }
      ...
      if (req.admin.role !== "SUPER_ADMIN") {
        return res
          .status(403)
          .json({ message: "คุณไม่มีสิทธิ์สร้างบัญชีระดับ SUPER_ADMIN ได้" });
      }
      ...
      return res
        .status(500)
        .json({ message: "เกิดข้อผิดพลาดในการสร้างบัญชีผู้ใช้งาน" });
    } finally {
      if (connection) connection.release();
    }
  },
...
app.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const [checkUser] = await connection.execute(
        "SELECT id, password FROM officers WHERE id = ?",
        [officerId],
      );
      if (checkUser.length === 0) {
        return res
          .status(404)
          .json({ message: "ไม่พบข้อมูลเจ้าหน้าที่ที่ต้องการแก้ไข" });
      }
      ...
      if (!username || !fullname || !role) {
        return res
          .status(400)
          .json({
            message: "กรุณากรอกข้อมูล Username, ชื่อ-สกุล และ Role ให้ครบถ้วน",
          });
      }
      ...
      const [existingUser] = await connection.execute(
        "SELECT id FROM officers WHERE username = ? AND id != ?",
        [username, officerId],
      );
      if (existingUser.length > 0) {
        return res
          .status(400)
          .json({ message: "Username นี้มีคนอื่นใช้ไปแล้ว" });
      }
      ...
      return res
        .status(200)
        .json({ message: "แก้ไขข้อมูลเจ้าหน้าที่เรียบร้อยแล้ว" });
    } catch (error) {
      console.error("Update Officer Error:", error);
      return res
        .status(500)
        .json({ message: "เกิดข้อผิดพลาดในการแก้ไขข้อมูล" });
    } finally {
      if (connection) connection.release();
    }
  },
    ...
    try {
      ...
      if (req.admin.id && String(req.admin.id) === String(officerId)) {
        return res
          .status(400)
          .json({
            message: "ไม่อนุญาตให้ลบบัญชีของตัวเองในขณะที่กำลังใช้งานอยู่ครับ",
          });
      }
      ...
      if (checkUser.length === 0) {
        return res
          .status(404)
          .json({ message: "ไม่พบข้อมูลเจ้าหน้าที่ที่ต้องการลบ" });
      }

      await connection.execute("DELETE FROM officers WHERE id = ?", [
        officerId,
      ]);

      return res
        .status(200)
        .json({ message: "ลบข้อมูลเจ้าหน้าที่สำเร็จเรียบร้อยแล้ว" });
    } catch (error) {
      ...
      if (error.code === "ER_ROW_IS_REFERENCED_2") {
        return res.status(400).json({
          message:
            "ไม่สามารถลบเจ้าหน้าที่คนนี้ได้ เนื่องจากมีประวัติการทำงานผูกอยู่ในระบบ แนะนำให้เข้าไปแก้ไขสถานะเป็น 'ระงับการใช้งาน' (is_active = 0) แทนครับ",
        });
      }

      return res
        .status(500)
        .json({ message: "เกิดข้อผิดพลาดในการลบเจ้าหน้าที่" });
    } finally {
      if (connection) connection.release();
    }
  },
  ...
  async (req, res) => {
    try {
      ...
      if (action !== "APPROVED" && action !== "REJECTED") {
        ...
        throw new ValidationError(
          "Action ต้องเป็น APPROVED หรือ REJECTED",
          400,
        );
      }
      if (
      ...
      if (requestInfo.status !== "PENDING") {
        return res
          ...
          .json({
            message:
              "คำขอถูกตรวจสอบไปแล้ว สถานะปัจจุบันคือ " + requestInfo.status,
          });
      }
  ...
  async (req, res) => {
    ...
    try {
      if (!req.file) {
        return res
          .status(400)
          .json({ message: "กรุณาอัปโหลดไฟล์ Excel หรือ Csv" });
      }
      ...
      const data = xlsx.utils.sheet_to_json(sheet, {
        raw: false, // บังคับให้อ่านค่าที่ Format แล้ว (จะได้ข้อความ "02/15/2002")
        dateNF: "yyyy-mm-dd",
      });
      ...
      if (data.length === 0) {
        return res
          .status(400)
          .json({ message: "ไม่พบข้อมูลในไฟล์หรือไฟล์ว่างเปล่า" });
      }
      ...
      const [zoneRows] = await connection.execute(
        `SELECT id, location_name FROM inmate_location`,
      );
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        const admission_date =
          row["วันรับโทษ"] || row["admission_date"] || null;
        ...
        if (!inmate_id_card || !inmate_id || !firstname || !lastname) {
          errorList.push(
            `แถวที่ Excel ${i + 2} : ข้อมูลไม่ครบ (ขาดเลขบัตรประชาชน รหัสนักโทษ หรือ ชื่อ-นามสกุล)`,
          );
          continue;
        }
        if (birthdate) {
          ...
          if (!isValidDate) {
            return res.status(400).json({
              message:
                "รูปแบบวันเกิดไม่ถูกต้อง หรือวันที่ไม่มีอยู่จริง กรุณาใช้ YYYY-MM-DD",
            });
          }
        }
        ...
        if (zoneText) {
          ...
          if (!finalZoneId) {
            errorList.push(
              `แถวที่ ${i + 2}: ไม่พบข้อมูลแดนที่ชื่อ "${zoneText}" ในระบบ กรุณาตรวจสอบการสะกดคำ`,
            );
...
app.get("/admin/devices", checkAPI_key, checkAdminAuth, async (req, res) => {
  try {
    ...
    const [devices] = await db.execute(
      `SELECT id, device_name, platforms FROM devices`,
    );
...
app.post(
  "/admin/visit-slots/generate-advanced",
  checkAPI_key,
  checkAdminAuth,
  checkRole(["SUPER_ADMIN", "REGISTRAR"]),
  async (req, res) => {
    try {
      ...
      if (!schedules || !Array.isArray(schedules) || schedules.length === 0) {
        return res
          .status(400)
          .json({ message: "กรุณาส่งข้อมูลตารางเวลาอย่างน้อย 1 รายการ" });
      }
      ...
      for (let i = 0; i < schedules.length; i++) {
        ...
        if (!date || !start_time || !end_time || !duration_minutes || !capacity)
          continue;
        ...
        while (currentTotalMinutes + duration_minutes <= totalEndMinutes) {
          ...
          const isDuplicate =
            existingSlotsSet.has(slotKey) || newSlotsSet.has(slotKey);
          ...
          previewList.push({
            ...
            status: isDuplicate
              ? "DUPLICATE (ข้ามการสร้าง)"
              : "NEW (พร้อมสร้าง)", // 🌟 บอกแอดมินชัดๆ!
          });
      ...
      if (slotsToInsert.length === 0) {
        return res.status(400).json({
          message:
            "รอบเวลาที่คุณต้องการสร้าง มีอยู่ในระบบทั้งหมดแล้ว (ไม่มีการสร้างใหม่)",
        });
      }
...
app.get(
  ...
  checkAPI_key,
  checkAdminAuth,
  checkRole(["SUPER_ADMIN", "REGISTRAR", "COMMANDER"]),
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "เกิดข้อผิดพลาดในการดึงข้อมูลรอบการเยี่ยม" });
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      const [existingSlot] = await db.execute(
        `SELECT current_booking FROM visit_slot WHERE id = ?`,
        [slotId],
      );
      ...
      if (existingSlot.length === 0) {
        return res
          .status(404)
          .json({ message: "ไม่พบรอบเวลานี้ในระบบ หรือถูกลบไปแล้ว" });
      }
      ...
      if (error.code === "ER_ROW_IS_REFERENCED_2") {
        return res.status(400).json({
          message:
            "ไม่สามารถลบได้ เนื่องจากข้อมูลรอบเวลานี้ผูกกับประวัติการจองในระบบแล้ว",
        });
      }
...
// ==========================================
app.get(
  "/admin/visit-bookings",
  checkAPI_key,
  checkAdminAuth,
  checkRole(["SUPER_ADMIN", "REGISTRAR", "VISITATION", "COMMANDER"]),
  async (req, res) => {
    try {
    ...
    try {
      ...
      if (!firstname || !lastname || !inmate_number) {
        return res
          .status(400)
          .json({
            message: "กรุณากรอก ชื่อ, นามสกุล และ รหัสนักโทษ ให้ครบถ้วน",
          });
      }
      if (id_card.length != 13) {
        return res
          .status(400)
          .json({ message: "กรุณากรอกเลขบัตรประชาชนให้ครบ 13 หลัก" });
      }
      if (birthdate) {
        // ใส่พารามิเตอร์ตัวที่ 3 เป็น true เพื่อเปิดโหมด Strict Validation
        const isValidDate = dayjs(birthdate, "YYYY-MM-DD", true).isValid();
      ...
      res.status(201).json({
        message:
          existingInmate.length > 0
            ? "อัปเดตประวัตินักโทษเดิมสำเร็จ"
            : "เพิ่มข้อมูลนักโทษใหม่สำเร็จ",
...
app.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const [checkInmate] = await connection.execute(
        `SELECT id FROM inmate WHERE id = ?`,
        [inmateId],
      );
      if (checkInmate.length === 0) {
        ...
        return res
          .status(404)
          .json({ message: "ไม่พบข้อมูลนักโทษที่ต้องการแก้ไข" });
      }
      ...
      const zoneMap = {};
      zoneRows.forEach(
        (zone) => (zoneMap[zone.location_name.toString().trim()] = zone.id),
      );
      ...
      prefixRows.forEach(
        (item) =>
          (prefixMap[item.prefixes_nameTh.toString().trim()] =
            item.id_prefixes),
      );
      ...
      const [checkIncarc] = await connection.execute(
        `SELECT id FROM incarcerations WHERE inmate_rowID = ?`,
        [inmateId],
      );
      ...
      if (error.code === "ER_DUP_ENTRY") {
        return res.status(400).json({
          message:
            "ข้อมูลนี้ (เช่น เลขบัตร หรือ รหัสนักโทษ) มีซ้ำในระบบแล้ว กรุณาตรวจสอบอีกครั้ง",
        });
      }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (checkInmate.length === 0) {
        ...
        return res
          .status(404)
          .json({ message: "ไม่พบข้อมูลนักโทษที่ต้องการลบ" });
      }

      // 2. ลบข้อมูลจากตารางลูก (incarcerations) ก่อน
      await connection.execute(
        `DELETE FROM visit_booking WHERE inmate_id = ?`,
        [inmateId],
      );
      await connection.execute(
        `DELETE FROM user_inmate_relationship WHERE inmateId = ?`,
        [inmateId],
      );

      await connection.execute(
        `DELETE FROM incarcerations WHERE inmate_rowID = ?`,
        [inmateId],
      );
      ...
      if (error.code === "ER_ROW_IS_REFERENCED_2") {
        return res.status(400).json({
          message:
            "ไม่สามารถลบได้ เนื่องจากนักโทษคนนี้มีประวัติผูกกับการจองคิวเยี่ยม (visit_booking) แล้ว",
        });
      }
    ...
    try {
      ...
      if (duplicateCheck.length > 0) {
        return res
          ...
          .json({
            message:
              "ไม่สามารถแก้ไขได้! วันและเวลานี้ของตู้ที่เลือก มีการเปิดรอบไว้แล้ว (คิวชนกัน)",
          });
      }
...
app.get(
  ...
  checkAPI_key,
  checkAdminAuth,
  checkRole(["SUPER_ADMIN", "COMMANDER"]),
  async (req, res) => {
    try {
      const startDate =
...
app.get("/api/dashboard/export", async (req, res) => {
  try {
    ...
    if (format === "csv") {
      ...
      csv +=
        "รหัสจอง,วันที่,เวลาเริ่ม,เวลาจบ,สถานะ,รหัสผู้ต้องขัง,ชื่อผู้ต้องขัง,ชื่อญาติ\n";
    ...
    if (format === "excel") {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=report_${reqStartDate}_to_${reqEndDate}.xlsx`,
      );
    ...
    if (format === "pdf") {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=report_${reqStartDate}_to_${reqEndDate}.pdf`,
      );
      doc.pipe(res);
...
app.put(
  "/admin/visit-bookings/:id/status",
  checkAPI_key,
  checkAdminAuth,
  checkRole(["SUPER_ADMIN", "VISITATION", "COMMANDER"]),
  async (req, res) => {
    try {
      ...
      if (!allowedStatuses.includes(status)) {
        throw new ValidationError(
          "สถานะไม่ถูกต้อง (ต้องเป็น COMPLETED หรือ NO_SHOW เท่านั้น)",
        );
      }

      // 2. ค้นหาข้อมูลการจองปัจจุบัน
      const [rows] = await db.execute(
        "SELECT status, relative_user_id, inmate_id FROM visit_booking WHERE id = ?",
        [bookingId],
      );
      if (rows.length === 0) {
      ...
      await db.execute("UPDATE visit_booking SET status = ? WHERE id = ?", [
        status,
        bookingId,
      ]);
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (device.length === 0) {
        return res
          .status(404)
          .json({ message: "ไม่พบข้อมูลอุปกรณ์ที่ต้องการ" });
      }
    ...
    try {
      ...
      if (!device_name || !platforms || platforms.length === 0) {
        return res
          .status(400)
          .json({
            message: "กรุณากรอกชื่ออุปกรณ์ และเลือกแพลตฟอร์มอย่างน้อย 1 อย่าง",
          });
      }
      ...
      if (platformsStr.includes("ZOOM")) {
        if (!zoom_account_id || !zoom_client_id || !zoom_client_secret) {
          return res
            ...
            .json({
              message:
                "คุณเลือกแพลตฟอร์ม ZOOM กรุณากรอกข้อมูล API Key ของ Zoom ให้ครบถ้วน",
            });
        }
      }
      if (platformsStr.includes("LINE")) {
        if (!line_oa_id) {
          return res
            .status(400)
            .json({ message: "คุณเลือกแพลตฟอร์ม LINE กรุณากรอก LINE OA ID" });
        }
      }
      ...
      return res
        .status(500)
        .json({ message: "เกิดข้อผิดพลาดในการเพิ่มอุปกรณ์" });
    } finally {
      if (connection) connection.release();
    }
  },
...
app.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const [checkDevice] = await connection.execute(
      ...
      if (checkDevice.length === 0) {
        return res
          .status(404)
          .json({ message: "ไม่พบข้อมูลอุปกรณ์ที่ต้องการแก้ไข" });
      }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (checkDevice.length === 0) {
        return res
          .status(404)
          .json({ message: "ไม่พบข้อมูลอุปกรณ์ที่ต้องการลบ" });
      }
      ...
      return res
        .status(200)
        .json({ message: "ลบข้อมูลอุปกรณ์สำเร็จเรียบร้อยแล้ว" });
    } catch (error) {
      ...
      if (error.code === "ER_ROW_IS_REFERENCED_2") {
        return res.status(400).json({
          message:
            "ไม่สามารถลบอุปกรณ์นี้ได้ เนื่องจากมีประวัติการจองคิวผูกอยู่ แนะนำให้เปลี่ยนสถานะเป็น 'ปิดปรับปรุง (MAINTENANCE)' แทนครับ",
        });
      }
...
app.get("/", async (req, res) => {
  try {
    const result = await db.execute(
      "SELECT * FROM user_inmate_relationship LIMIT 1",
    );
    res.json({ message: "Welcome to my API", data: result[0] });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Internal Server Error" });
  }
...
app.post("/check-idcard", checkAPI_key, async (req, res) => {
  try {
    ...
    const [check_SQL] = await db.execute(
      "SELECT id_card, firstname, lastname FROM user WHERE id_card = ?;",
      [trimmedID_Card],
    );
...
app.post("/register", checkAPI_key, async (req, res) => {
  ...
  try {
    ...
    if (hasnonAscii) {
      throw new ValidationError(
        "Password ต้องเป็นตัวอักษรภาษาอังกฤษหรือตัวเลขเท่านั้น",
      );
    }
    if (trimmedPassword == "") {
      throw new ValidationError("Password ห้ามเป็นค่าว่าง หรือ มีเว้นวรรค");
    ...
    if (trimmedPassword.length < 8) {
      throw new ValidationError(
        "Password นี้ต้องมีความยาวอย่างน้อย 8 ตัวอักษร",
      );
    }
    ...
    const [prefixRows] = await connection.execute(
      "SELECT id_prefixes FROM prefixes WHERE prefixes_nameTh = ?",
      [trimmedPrefix],
    );
    ...
    const [existingUser] = await connection.execute(
      "SELECT id_card FROM user WHERE id_card = ? FOR UPDATE;",
      [newUser.id_card],
    );
    if (existingUser.length > 0) {
      throw new ValidationError(
        "เลขบัตรประชาชนนี้มีบัญชีผู้ใช้งานในระบบแล้ว กรุณาเข้าสู่ระบบ",
      );
    }
    ...
    const sql =
      "INSERT INTO `user` (`id_card`,`prefixe_id`, `firstname`, `lastname`, `hashed_password`, `create_time`, `phone`, `is_active`, `last_active_at`) VALUES (?, ?, ?, ?, ?, NOW(), ?, 1, NULL);";
    ...
    return res.status(201).json({
      message:
        "สมัครสมาชิกสำเร็จ! คุณสามารถเข้าสู่ระบบและส่งคำขอผูกรายชื่อผู้ต้องขังได้เลย",
      data: {
        id: data.insertId,
...
app.post("/login", checkAPI_key,loginLimiter, async (req, res) => {
  try {
    ...
    const [rows] = await db.execute(
      "SELECT userId ,firstname,id_card, hashed_password ,is_active ,last_active_at FROM user WHERE id_card = ?",
      [trimmedID_Card],
    );
    if (rows.length === 0) {
    ...
    if (!passwordMatch) {
      throw new ValidationError(
        "เลขบัตรประชาชน หรือ รหัสผ่านของคุณไม่ถูกต้อง",
        401,
      );
    }
    ...
    const update_last_active = await db.execute(
      "UPDATE user SET last_active_at = NOW() WHERE userId = ? ",
      [data.userId],
    );
    ...
    if (device_token) {
      await db.execute(
        "INSERT INTO device (user_id,device_info,device_type,last_active_at) VALUES (?,?,?,NOW()) ON DUPLICATE KEY UPDATE user_id = VALUES(user_id), last_active_at = NOW()",
        [data.userId, device_token, device_type || "unknown"],
      );
    }
...
app.post("/admin/login", checkAPI_key, async (req, res) => {
  try {
    ...
    if (
      ...
      throw new ValidationError(
        "ชื่อผู้ใช้ หรือ รหัสผ่านของคุณไม่ถูกต้อง",
        400,
      );
    }
    ...
    const [rows] = await db.execute(
      "SELECT id,username,password,fullname,role,is_active FROM officers WHERE username = ?",
      [trimmedUsername],
    );
  ...
  (req, res) => {
    try {
      ...
      if (!fs.existsSync(filePath)) {
        return res
          .status(404)
          .json({ message: "ไม่พบรูปภาพนี้ในระบบ หรือรูปอาจถูกลบไปแล้ว" });
      }
...
app.get("/main", checkAPI_key, checkAuth, async (req, res) => {
  try {
    ...
    const [rows] = await db.execute(
      "SELECT u.id_card,u.firstname,u.lastname, p.prefixes_nameTh AS prefix_name FROM user AS u LEFT JOIN prefixes AS p ON u.prefixe_id = p.id_prefixes WHERE u.userId = ?",
      [myUserId],
    );
    if (rows.length === 0) {
...
app.get("/inmate_info", checkAPI_key, checkAuth, async (req, res) => {
  try {
    ...
    const inmateList = inmate.map((item) => ({
      ...
      allow_visit:
        item.allow_visit === 1
          ? "ผู้ต้องขังนี้สามารถรับการเยี่ยมชมได้"
          : "ผู้ต้องขังนี้ไม่สามารถรับการเยี่ยมชมได้ เนื่องจากมีเหตุผลบางประการ เช่น สุขภาพไม่ดี หรือ อยู่ในระหว่างการถูกลงโทษ",
    }));
...
app.get("/inmate_info/:id", checkAPI_key, checkAuth, async (req, res) => {
  try {
    ...
    if (rows.length === 0) {
      return res
        .status(404)
        .json({ message: "ไม่พบข้อมูลผู้ต้องขังที่เกี่ยวข้อง" });
    }
...
app.get("/slot/monthly", checkAPI_key, checkAuth, async (req, res) => {
  try {
    ...
    if (rows.length === 0) {
      return res
        .status(404)
        .json({ message: "ไม่พบข้อมูลช่องเวลาการเยี่ยมชมในเดือนและปีที่ระบุ" });
    }
    ...
    rows.forEach((row) => {
      ...
      calendarData[dateKey] = {
        ...
        seats_left: Math.max(
          0,
          Number(row.total_capacity) - Number(row.total_booked),
        ),
      };
    });
...
app.get("/slots/preview", checkAPI_key, checkAuth, async (req, res) => {
  try {
    ...
    if (rows.length === 0) {
      throw new ValidationError(
        "ไม่พบข้อมูลผู้ต้องขังที่เกี่ยวข้องหรือผู้ต้องขังนี้ไม่สามารถรับการเยี่ยมชมได้",
      );
    }
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        allow_visit: {
          ...
          message: isAllowVisit
            ? "ผู้ต้องขังนี้สามารถรับการเยี่ยมชมได้"
            : "ผู้ต้องขังนี้ไม่สามารถรับการเยี่ยมชมได้ เนื่องจากมีเหตุผลบางประการ เช่น สุขภาพไม่ดี หรือ อยู่ในระหว่างการถูกลงโทษทางวินัย",
        },
      },
    });
  } catch (error) {
...
app.get("/slots", checkAPI_key, checkAuth, async (req, res) => {
  ...
  try {
    ...
    if (exclude_booking_id) {
      const [excludeInfo] = await db.execute(
        `SELECT slot_id FROM visit_booking WHERE id = ? AND status NOT IN('CANCELLED','REJECTED')`,
        [exclude_booking_id],
      );
    ...
    if (rows.length === 0) {
      return res
        .status(404)
        .json({ message: "ไม่พบช่องเวลาการเยี่ยมชมในวันที่ระบุ" });
    }
    ...
    console.log(
      "ข้อมูลช่องเวลาการเยี่ยมชมที่จัดกลุ่มตามอุปกรณ์: ",
      Object.values(deviceName_Set),
    );
    res.status(200).json({
...
app.get("/booking-preview", checkAPI_key, checkAuth, async (req, res) => {
  try {
    ...
    if (existingBooking.length > 0) {
      ...
      return res.status(400).json({
        message:
          "ผู้ต้องขังรายนี้มีคิวเยี่ยมในวันที่เลือกแล้ว (สิทธิ์การเยี่ยม 1 ครั้ง/วัน) กรุณาเลือกวันอื่นครับ",
      });
    }
    ...
    if (data.Quota_Used >= 2) {
      throw new ValidationError(
        "คุณได้ใช้สิทธิ์การจองช่องเวลาการเยี่ยมชมสำหรับผู้ต้องขังนี้ครบตามโควต้าประจำเดือนแล้ว",
      );
    }

    console.log(
      "ผลลัพธ์การดึงข้อมูลช่องเวลาการเยี่ยมชมสำหรับพรีวิวการจอง: ",
      slotRows[0],
    );
    ...
    const bookingToken = jwt.sign(BookingPayload, process.env.JWT_SECRET, {
      expiresIn: "10m",
    });
...
app.post("/booking", checkAPI_key, checkAuth, async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      slotData.booking_status !== "CANCELLED" &&
      slotData.booking_status !== "REJECTED"
    ) {
    ...
    console.log(
      "ช่องเวลาการเยี่ยมชมนี้ยังมีที่ว่างอยู่ สามารถทำการจองได้",
      slotData.platforms,
    );
    const randomStr = Math.random().toString(36).substring(2, 8).toUpperCase();
    const bk_type =
      slotData.platforms === "ZOOM"
        ? "ZM"
        : slotData.platforms === "WEBRTC"
          ? "RTC"
          : "LN";
    ...
    if (slotData.platforms === "ZOOM") {
      ...
      console.log(
        "วันที่และเวลาที่ฟอร์แมตสำหรับการสร้างการประชุม Zoom: ",
        formatedDate,
      );
...
app.put("/booking/:id/cancel", checkAPI_key, checkAuth, async (req, res) => {
  ...
  try {
    ...
    if (diffDays <= 1) {
      throw new ValidationError(
        "ไม่สามารถยกเลิกการจองได้เนื่องจากเหลือเวลาน้อยกว่า 1 วันก่อนวันเยี่ยมชม",
      );
    }
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const booking_id = req.params.id;
      const todayTime = new Date().toLocaleDateString("en-CA", {
        timeZone: "Asia/Bangkok",
      });
      ...
      if (bookingRows.length === 0) {
        throw new ValidationError(
          "ไม่พบการจองที่ระบุหรือไม่สามารถเปลี่ยนรอบการเยี่ยมชมได้",
        );
      }
      const bookingData = bookingRows[0];

      const thaiDate = new Date(bookingData.visit_date).toLocaleDateString(
        "th-TH",
        {
          year: "numeric",
          month: "long",
      ...
      if (diffHours < 24) {
        throw new ValidationError(
          "ไม่สามารถเปลี่ยนรอบการเยี่ยมชมได้เนื่องจากเหลือเวลาน้อยกว่า 1 วันก่อนวันเยี่ยมชม",
        );
      }
      console.log(
        "ข้อมูลการจองที่ตรวจสอบสำหรับการเปลี่ยนรอบการเยี่ยมชม: ",
        bookingData,
      );
      return res.status(200).json({
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const oldDateTH = new Date(oldInfo.visit_date).toLocaleDateString(
        "th-TH",
        {
          year: "numeric",
          month: "long",
      ...
      const [new_slot] = await db.execute(
        `SELECT vs.visit_date,TIME_FORMAT(vs.starts_at,'%H:%i') AS starts_at,TIME_FORMAT(vs.ends_at,'%H:%i') AS ends_at,vs.status,
      ...
      if (
        newSlot.status === "FULL" ||
        Number(newSlot.current_booking) >= Number(newSlot.capacity)
      ) {
        throw new ValidationError("รอบการจองนี้ถูกจองแล้ว");
      }
      const newDateTH = new Date(newSlot.visit_date).toLocaleDateString(
        "th-TH",
        {
          year: "numeric",
          month: "long",
      ...
      const rescheduleToken = jwt.sign(payload, process.env.JWT_SECRET, {
        expiresIn: "5m",
      });
      res.status(200).json({
...
app.post("/booking/reschedule", checkAPI_key, checkAuth, async (req, res) => {
  ...
  try {
    ...
    if (
      decodedToken.action !== "RESCHEDULE" ||
      decodedToken.userId !== user_id
    ) {
    ...
    if (Number(old_slot.slot_id) === Number(new_slot_id)) {
      throw new ValidationError(
        "คุณเลื่อนการจองนี้ไปแล้ว ไม่สามารถเลื่อนซ้ำได้",
      );
    }
    ...
    if (
      ...
      newSlotRows[0].status != "OPEN" ||
      Number(newSlotRows[0].current_booking) >= Number(newSlotRows[0].capacity)
    ) {
    ...
    await connection.execute(
      `UPDATE visit_booking SET slot_id = ?,
                meeting_id = ?,
                meeting_password = ?,
                join_url = ?,
                starts_url = ?,
    ...
    const [oldUpdate] = await connection.execute(
      `UPDATE visit_slot SET current_booking = current_booking - 1 WHERE id = ? AND current_booking > 0`,
      [old_slot.slot_id],
    );
    ...
    const [newUpdate] = await connection.execute(
      `UPDATE visit_slot SET current_booking = current_booking + 1 WHERE id = ? AND current_booking < capacity`,
      [newSlot.id],
    );
    ...
    if (newUpdate.affectedRows === 0) {
      ...
      throw new ValidationError(
        "ขออภัย คิวใหม่เต็มแล้วในขณะที่คุณกำลังทำรายการ",
      );
    }
...
app.get(
  ...
  checkAPI_key,
  checkAdminAuth,
  checkRole(["SUPER_ADMIN", "REGISTRAR", "VISITATION"]),
  async (req, res) => {
    try {
      ...
      let whereConditions = [
        "vb.status IN ('COMPLETED', 'NO_SHOW', 'CANCELLED', 'REJECTED')",
      ];
      let queryParams = [];
      ...
      res
        .status(500)
        .json({ message: "เกิดข้อผิดพลาดในการดึงข้อมูลประวัติย้อนหลัง" });
    }
  },
);
...
app.put(
  ...
  async (req, res) => {
    try {
      ...
      if (!meeting_link) {
        throw new ValidationError(
          "กรุณาระบุ meeting_link สำหรับการอัปเดตช่องเวลาการเยี่ยมชม",
        );
      }
      ...
      if (updateResult.affectedRows === 0) {
        throw new ValidationError(
          "ไม่พบการจองที่ระบุหรือไม่สามารถอัปเดตลิงก์ได้",
        );
      }
      res.status(200).json({
...
app.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (
        ["CANCELLED", "COMPLETED", "REJECTED", "CHECKED_IN"].includes(
          currentStatus,
        )
      ) {
        throw new ValidationError(
          "ไม่สามารถยกเลิกการจองนี้ได้เนื่องจากสถานะปัจจุบันคือ " +
            currentStatus,
        );
      }
      ...
      const thaiDate = new Date(bookingRows[0].visit_date).toLocaleDateString(
        "th-TH",
        {
          year: "numeric",
          month: "long",
      ...
      await connection.execute(
        ...
        [bookingRows[0].relative_user_id, cancelTitle, cancelMessage, "0"],
      );
